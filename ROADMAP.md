...
- Added the class-name slice and enemy-name slice as separate rename targets and exported them to dedicated CSVs.
...
- Added the API-style `character <name> rename <replacement>` command shape and kept `patch-character-name` as a compatibility alias.
...
- Documented the raw battery-save roadmap and the savestate caveat separately from ROM patching.
...
- Added the confirmed class-name slice and enemy-name slice as separate rename targets and started the same API-style `class <name> rename <replacement>` and `enemy <name> rename <replacement>` command paths for them.
- Added the confirmed item-name workflow and dialogue-speaker/NPC workflow as separate rename targets with the same API-style command shape and conservative repointing rules.
